...
## restore.py (SeedVR2 segment restoration — standard stage, default ALL segments)
...
- Run it AFTER the cut is locked (it repoints assign.json like a hand-patch — re-running assign_clips.py discards it), then re-render both orientations + re-run qc. Default is every segment — production optimizes for quality, not speed (Marcus, 2026-07-18); narrow with `--heroes`/`--segs` only under genuine VRAM/deadline constraints. Failed segments keep their originals; re-runs skip `__sr` segments, so redo rounds are incremental.
