...
**AI restoration is a standard stage — production optimizes for quality, not speed.** Once the cut is locked (grid clean, heroes verified), run `python3 $S/restore.py <workdir>` — SeedVR2 restores EVERY segment's source window (real detail recovery on compressed footage; ~2.7s/frame on the GPU, ~30–60 min per 15s edit, needs ~10GB free VRAM so run it without other heavy GPU jobs) and repoints assign.json — then re-render both orientations and re-run qc. Already-restored segments skip, so redo rounds only pay for changed segments. `--heroes`/`--segs i,j,k` narrow it only when a genuine constraint (VRAM contention, deadline) forces it.
...
Write `out/director_note.md` (choices + what you fixed between passes). **Default delivery is the hypebot Telegram bot** (token/chat in `~/.config/hypebot/secrets.env`): send **only the landscape render** (`sendVideo`; files ≥49MB get a ~46MB preview encode first), then the TikTok caption as its **own separate message** so it copy-pastes clean. The portrait master is NOT sent — both orientations stay full-res on disk under `~/Videos/hype/<date>/`. Never Taildrop unless explicitly asked.
