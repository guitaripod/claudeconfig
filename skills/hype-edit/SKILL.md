...
**Optional AI restoration for marquee moments**: once the cut is locked, `python3 $S/restore.py <workdir> --heroes` (or `--segs i,j,k`) runs SeedVR2 on those segments' source windows (compressed/soft sources gain real detail; ~2.7s/frame on the GPU, needs ~10GB free VRAM) and repoints assign.json — then re-render both orientations. Opt-in only; skip when sources are already crisp.
