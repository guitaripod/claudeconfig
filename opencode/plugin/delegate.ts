...
/** Runs one delegate subcommand for `args` in `cwd` and returns the text the tool call reports. */
async function runDelegate(args: DelegateArgs, cwd: string): Promise<string> {
      ...
      tools.add({
        ...
        execute: async (args: DelegateArgs, context: { sessionID: string }) => {
          try {
            ...
            return { content: await runDelegate(args, cwd) }
          } catch (error) {
